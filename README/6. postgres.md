...
Pod's are predominently designed to be ephemeral in kubernetes. Once they die, their history/volume/data is lost. If they stored a 10mb file on a volume, it's designed to dissapear at the end of the pods lifecycle. This lifecycle is known as a **deployment** in kubernetes. If the pod kept it's storage with it, it would be called a "**Stateful set**" (meaning the pod and it's data "set" are kept together/persisted). Set in this vague definition also includes a static name (e.g `postgres-0`) note that this number increments for each replica.

[More on statefulsets here](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)...
...
| Deployment type | Storage | Name/DNS | End of a lifecycle |
| ----------------| ------- | ------------------ |
| Deployment      | Volume/ephemeral | Lost/changed | Data lost |
| Statefulset.    | Persistent | Retained | Data kept and retained regardless of the pod's lifecycle |
...
### A Persistent Volume (PV) definition
...
| Field | Explanation |
| ----- | ----------- |
| [spec.capacity.storage](../manifests/postgres/storage.yaml#L7) | The size allocated to the storage (this is not the same as what the pod sees/wants). |
...
| [spec.claimRef.name](../manifests/postgres/storage.yaml#L15) | **IMPORTANT** - This MUST match the `<storage claim>-<app name>-0` of the pod |
...
### A Persistent Volume Claim (PVC) partial definition
...
It's worth noting this is a vital addition to a statefulset, not independent yaml. Takeaway this and you have a standard deployment.
