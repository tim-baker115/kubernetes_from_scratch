...
So we now have two pods; this makes sense - we've got two different containers running. Let's have a look at the one with the `CrashLoopBackOff` as that's a new error:
...
This error implies that the mount point is wrong, I imagine it's using / or something by default. I'm going to add a directory to my yaml file to fix it (this will go under the containers spec):
...
Even though this is actually a deployment, it shouldn't represent a problem. The deployment is set to pull the correct image and meet the criteria of the last edited deployment. The `ContainerCreating` "pod" is just hanging around until someone says "go away".
    ...
    steps:
      ...
      - name: Check kubectl
        run: |
          ...
          /runner/kubectl get nodes
          /runner/kubectl get pods -n kube-system -o wide -A
```
...
...Except we have an error in github now when pushed.

```
line 3: kubectl: command not found Error: Process completed with exit code 127.
```

So this has an error when running kubectl, there's no binary for that. That makes sense, why would there be? We'll add an init-container  to the deployment to include kubectl. More on the differences between a sidecar and an [init-container](https://kubernetes.io/docs/concepts/workloads/pods/init-containers/). To summarise though; an init-container will execute before the container. Unlike a sidecar it doesn't run continually, it just runs a simple process and then the main container start. Interesting concept. So under the spec we'll add this:
    ...
    spec:
      ...
      - name: install-kubectl
        ...
        - name: runner-workdir
          mountPath: /runner
```
This will copy kubectl binary from bitnami/kubectl:latest and drop it into /runner. My yaml file is now complete and no further changes will be applied to it [githubrunner.yaml](../manifests/githubrunner/githubrunner.yaml). I've pushed the latest version to git at this point.
...
This is a permission thing ([more on RBAC and some examples from kubernetes](https://kubernetes.io/docs/reference/access-authn-authz/rbac/)). We're going to make a `ClusterRole` and a `ClusterRoleBinding` ([more on these concepts here](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#rolebinding-and-clusterrolebinding)) first the ClusterRole:
